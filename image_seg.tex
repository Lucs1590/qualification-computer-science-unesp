...
Métodos mais modernos que seguem essa linha serão explicados detalhadamente nas seções \ref{semantic:semantic}, \ref{instance:instance} e \ref{panoptic:panoptic}.


\subsection{Considerações Finais da Seção}

Na tabela \ref{segment:table:1} é possível visualizar as técnicas comentadas no decorrer da seção assim como uma breve descrição de seus usos, suas vantagens e desvantagens.
...
\begin{table}[!ht]
    ...
    Segmentação Baseada em Regiões      & Concentra-se em encontrar e agrupar valores semelhantes sendo separados por um limiar.              & \begin{tabular}[c]{@{}p{3.8cm}@{}}- Possui baixa complexidade e alto desempenho;\\ - Não requer pré-processamento;\\ - Recomendado para alto contraste entre fundo e objeto de interesse.\end{tabular}         & - Omissão de detalhes quando não há um contraste significante.                                                                                                              \\ \hline
    Segmentação por Bordas              & Realizado com base na detecção de descontinuidade, detectando bordas e definir um limite do objeto. & - Recomendável para imagens com bons contrastes entre os objetos.                                                                                                                                       & - Perda de detalhes para imagens com muitas bordas, ruidosas ou com baixo contraste entre os objetos.                                                                       \\ \hline
    Segmentação Baseada em Agrupamentos & Divide os pixels da imagem em k clusters homogêneos e  exclusivos obtendo, assim, as segmentações.  & \begin{tabular}[c]{@{}p{3.8cm}@{}}- Conceitos já estabelecido;\\ - Recomendado para aplicações em tempo real;\\ - Bom desempenho para pequenos datasets;\\ - Não precisa de tempo de treinamento.\end{tabular} & \begin{tabular}[c]{@{}p{3.8cm}@{}}- Pode demandar muito tempo para encontrar a minimização da função de custo;\\ - Não é adequado para agrupar clusters não convexos.\end{tabular} \\ \hline
    Segmentação com Redes Neurais             & Normalmente relacionado a algoritmos de aprendizado profundo e CNNs.                                & \begin{tabular}[c]{@{}p{3.8cm}@{}}- Conceito já estabelecido;\\ - Abordagem simples e flexível;\\ - Disponibilizado por bibliotecas prontas em Python.\end{tabular}                                            & - O treinamento do modelo para consome muito tempo e recursos.                                                                                                              \\ \hline
    \end{tabular}
...
Na seção \ref{segment:segment} foi comentado sobre os algoritmos comumente utilizados para segmentações, trabalhando desde algoritmos mais simples como o de \textit{threshold segmentation} (\ref{segment:region}) até o nível de complexidade de técnicas de redes neurais artificias \ref{segment:neural}. Porém como comentado por \cite{Ghosh2019} e por \cite{Minaee2021}, hodiernamente há segmentações que utilizam de \textit{deep learning} e possuem ótimas abordagens, as quais estamos denominando como \textbf{segmentações modernas} e serão exploradas nas próximas seções.